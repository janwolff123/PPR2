...
server <- function(input, output, session) {
  ...
  observeEvent(input$show_instructions, {
    showModal(
      modalDialog(
        ...
        HTML("
          <ul>
            <li><strong>Aufenthalte hochladen:</strong> Eine CSV-Datei mit den Spalten Fallnummer, PID, Name, Kürzel, Datum (Tag).</li>
  ...
  # Fortschrittsanzeige
  progress <- reactiveVal("Warten auf Upload der Dateien...") # Initialstatus
  output$progressStatus <- renderText({ progress() })
  ...
  output$downloadData <- downloadHandler(
    ...
    content = function(file) {
      progress("Berechnung abgeschlossen. Exportieren der Ergebnisse...")
      data <- processed_data()
      wb <- createWorkbook()
