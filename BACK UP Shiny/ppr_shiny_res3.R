...
server <- function(input, output, session) {
  ...
  observeEvent(input$show_instructions, {
    showModal(
      modalDialog(
        ...
        HTML("
          <ul>
            <li><strong>Allgmemein</strong> Bitte alle als CSV-UTF-8 bereitstellten. Spaltenüberschriften in die erste Zeile. </li>
            <li><strong>Aufenthalte hochladen:</strong> Datei entsprechend Absprachen aus L3.</li>
            <li><strong>PPR-Kategorien hochladen:</strong> Datei entsprechend Absprachen aus TNL. Bei Kürzel richtigen Umlaut. </li>
            <li><strong>Steuerungsdatei hochladen:</strong> Die abgestimmte selbst erstellte CSV. </li>
            <li><strong>Station hochladen:</strong> Die abgestimmte selbst erstellte CSV. </li>
            <li><strong>Neugeborene hochladen:</strong> Die abgesprochenen Neugeborenen aus Orbis. Datei wie abgestimmt. </li>
          </ul>
        "),
        easyClose = TRUE,
  ...
  processed_data <- reactive({
    req(input$TNL, input$L3, input$Steuerung, input$Station, input$Neugeborene)
    ...
    tagesliste3 <- tagesliste2 %>%
      ...
      mutate(imputed=ifelse(is.na(Kürzel2),
                            1,
                            0))%>%
      group_by(Station)%>%
    ...
    Station<-
      ...
      group_by(Station2, Kategorie, Ausfall1, Ausfall2, Ausfall3, nachtdienst_quote)%>%
    ...
    ppr_ergebnis_minuten<-
      ...
      summarise(ppr_tag=sum(Gesamtwert, na.rm=T),
                ...
                Patiententage=n())%>%
      left_join(Station%>%
                  select(Station2, arbeitszeit_tag_vk, nachtdienst_quote),
                by="Station2")%>%
      mutate(VK_tag=ppr_tag/arbeitszeit_tag_vk,
             Minuten_nacht=ifelse(Patienten_mitternacht/nachtdienst_quote<1,
                                  60*8,
                                  Patienten_mitternacht/nachtdienst_quote*60*8),
